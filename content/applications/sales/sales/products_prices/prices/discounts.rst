...
--> Settings`. Click the :guilabel:`Discounts` checkbox in the :guilabel:`Pricing` section, then
click :guilabel:`Save`.

.. image:: discounts/discounts-option.png
...
discount for each product line and the new price will automatically be calculated in the
...
- :guilabel:`Global Discount`: Add a discount product line to the order, which has the cumulative
  ...
  .. example::
     In this example, since the product total is $4,200, and there is a 10% global discount, a
     global discount line is added to the order with a -$420 value (which is 10% of $4,200).

     .. image:: discounts/global-discount-option-so.png
...
- :guilabel:`Fixed Amount`: Add a monetary amount to the :guilabel:`Discount` field. Once applied,
  ...
  .. example::
     In this example, the fixed amount of $20 appears as a product line, with a negative
     :guilabel:`Unit Price`.
