...
of tests suites or scheduled actions so that it can be isolated.
...
In scheduled actions, you should rollback the changes if you catch errors and
wish to continue. You run in a separate transaction, so you may directly
rollback or commit when signaling the progress.

If you are handling framework exceptions, you must use **savepoints**
